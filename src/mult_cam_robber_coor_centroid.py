"""
Multi-Camera Robber Tracker - CENTROID TRACKER VERSION
For comparison with SORT tracker version

Key Difference: Uses CentroidTracker instead of SORT for object tracking
This version is for metrics comparison to demonstrate SORT's improvements
"""
...
from centroid_tracker import CentroidTracker  # CHANGED: Using CentroidTracker instead of SORT
import socket
import json
...
class TrackClassifier:
    """
    Maps Centroid tracker IDs to vehicle classes (Car, Police, Robber)
    ...
    def update(self, tracks, detections, class_ids, max_distance=50.0):
        """
        Match Centroid tracks to YOLO detections to assign vehicle classes

        Args:
            tracks: Centroid tracker output {track_id: (cx, cy)}
        ...
        for i, track_id in enumerate(track_ids):
            track_centroid = np.array(tracks[track_id])
...
# =============================================================================
class CameraHandler:
    """Handles multi-camera video streams with Centroid tracking"""
    ...
    def __init__(self):
        ...
        # Centroid trackers (one per camera) - CHANGED FROM SORT
        self.trackers = {}
        ...
        cv2.namedWindow("Camera 1 (CENTROID)", cv2.WINDOW_NORMAL)
        cv2.resizeWindow("Camera 1 (CENTROID)", *self.window_size)
        cv2.namedWindow("Camera 2 (CENTROID)", cv2.WINDOW_NORMAL)
        cv2.resizeWindow("Camera 2 (CENTROID)", *self.window_size)
    ...
    def process_data(self, data, addr, port):
        ...
        if addr not in self.buffers:
            ...
            # Create Centroid tracker for this camera - CHANGED FROM SORT
            self.trackers[addr] = CentroidTracker(maxDisappeared=20)
        ...
        if all(self.buffers[addr]):
            ...
            if img_np is not None:
                ...
                # === DETECTION: Run YOLO + Centroid Tracker ===
                results = model(img_np, conf=0.4)

                detections = []
                class_ids = []
                rects = []  # Bounding boxes for centroid tracker
                ...
                # Update Centroid tracker - CHANGED FROM SORT
                # Centroid tracker returns {track_id: (cx, cy)}
                tracked_objects = self.trackers[addr].update(rects)
                ...
                for track_id, centroid in tracked_objects.items():
                    ...
                    # Convert centroid tuple to numpy array
                    centroid_np = np.array([float(centroid[0]), float(centroid[1])])

                    # Update velocity tracker
                    self.velocity_trackers[addr].update(track_id, centroid_np)

                # Update track classifier
                self.track_classifiers[addr].update(tracked_objects, detections, class_ids)
                ...
                # === VISUALIZATION ===
                self._draw_tracks(img_np, tracked_objects, addr)
    ...
    def _process_prediction_and_interception(self, active_track_ids, addr, port):
        ...
        print(f"✓ Sent tracking data to Unity (CENTROID)")

        return tracking_data

    def _draw_tracks(self, img, tracked_objects, addr):
        """Draw tracking visualization with color-coded vehicle classes (only police and robber)"""
        for track_id, centroid in tracked_objects.items():
            track_id = int(track_id)
            ...
            # ONLY DRAW POLICE AND ROBBER - skip regular cars
            if vehicle_class == 0:  # Regular car - skip
                continue
            ...
            # Draw centroid
            centroid_pt = (int(centroid[0]), int(centroid[1]))
            cv2.circle(img, centroid_pt, 6, color, -1)

            # Draw ID, class, and speed
            text = f"{label} {track_id} ({speed:.1f}px/s)"
            cv2.putText(img, text, (centroid_pt[0] - 10, centroid_pt[1] - 10),
                       cv2.FONT_HERSHEY_SIMPLEX, 0.5, color, 2)

            # Draw velocity arrow
            if speed > 5:
                arrow_end = (int(centroid_pt[0] + velocity[0] * 0.1),
                           int(centroid_pt[1] + velocity[1] * 0.1))
                cv2.arrowedLine(img, centroid_pt, arrow_end, color, 2)
            ...
            if self.camera_views:
                for i, addr in enumerate(self.camera_views.keys()):
                    ...
                    if i == self.current_camera_index:
                        cv2.imshow(f"Camera {i + 1} (CENTROID)", self.camera_views[addr])
                    else:
                        cv2.imshow(f"Camera {i + 1} (CENTROID)", last_frames[addr])
...
async def main():
    ...
    print("="*60)
    print("Multi-Camera Robber Tracker - CENTROID VERSION")
    print("="*60)
    print("✓ Listening for camera data on ports 8081, 8082")
    print("✓ Detection: YOLOv8 + Centroid Tracker")
